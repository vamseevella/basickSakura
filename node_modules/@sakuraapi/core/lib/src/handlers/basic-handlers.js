...
function getRouteHandler(req, res, next) {
    ...
    model
        ...
        resLocals.status = http_status_1.OK;
...
function getAllRouteHandler(req, res, next) {
    ...
    model
        ...
        resLocals.send(http_status_1.OK, response);
        next();
    })
        .catch((err) => {
        console.log(err);
...
function putRouteHandler(req, res, next) {
    ...
    if (!req.body || typeof req.body !== 'object') {
        resLocals
            .send(http_status_1.BAD_REQUEST, {
            body: req.body,
            error: 'invalid_body'
        });
        return next();
    ...
    model
        ...
        .then((obj) => {
        if (!obj) {
            resLocals.status = http_status_1.NOT_FOUND;
            return next();
        }
        obj
            .save(changeSet)
...
function postRouteHandler(req, res, next) {
    ...
    obj
        ...
        if (err.name === 'MongoError') {
            switch (err.code) {
                case 11000:
                    ...
                    break;
                default:
                    err.status = http_status_1.SERVER_ERROR;
                    resLocals.send(http_status_1.SERVER_ERROR, {
        ...
        if (err.status === http_status_1.SERVER_ERROR) {
            console.log(err);
        }
        next();
    });
...
function deleteRouteHandler(req, res, next) {
    ...
    model
        .removeById(id)
        .then((result) => {
        resLocals.send(http_status_1.OK, {
            n: (result.result || {}).n || 0
        });
        next();
    })
...
function sanitizedUserInput(res, errMessage, fn) {
    try {
        ...
        if (err instanceof SyntaxError
            ...
            err.status = http_status_1.BAD_REQUEST;
        }
        else {
            res
                .locals
