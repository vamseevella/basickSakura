...
const _injectable_1 = require("../@injectable");
const helpers_1 = require("../helpers");
const db_1 = require("./db");
const errors_1 = require("./errors");
const format_from_json_1 = require("./format-from-json");
const format_to_json_1 = require("./format-to-json");
...
function Model(modelOptions) {
    ...
    return (target) => {
        const newConstructor = new Proxy(target, {
            construct: (t, args, nt) => {
                const diArgs = _injectable_1.getDependencyInjections(target, t, target[exports.modelSymbols.sapi]);
...
function fromJson(json, context = 'default') {
    const modelName = this.name;
    ...
    const formatFromJsonMeta = Reflect.getMetadata(format_from_json_1.formatFromJsonSymbols.functionMap, resultModel);
...
function fromJsonToDb(json, context = 'default') {
    const modelName = this.name;
    ...
    function mapJsonToDb(model, jsonSrc, result) {
        ...
        for (const contextKey of jsonByPropertyName.keys()) {
            ...
            const key = jsonMeta[json_1.jsonSymbols.propertyName];
...
function toJson(context = 'default') {
    debug.normal(`.toJson called, target '${this.constructor.name}'`);
    let json = mapModelToJson(this);
    const formatToJson = Reflect.getMetadata(format_to_json_1.formatToJsonSymbols.functionMap, this);
    if (formatToJson) {
    ...
    function mapModelToJson(source) {
        ...
        const dbOptionsByPropertyName = Reflect.getMetadata(db_1.dbSymbols.dbByPropertyName, source);
        const privateFields = Reflect
            .getMetadata(private_1.privateSymbols.sakuraApiPrivatePropertyToFieldNames, source)
            || new Map();
        for (const key of Object.getOwnPropertyNames(source)) {
